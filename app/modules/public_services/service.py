...
class PublicServiceEngine(PublicServiceProvider):
    def __init__(
        ...
        ai_gateway: Any,  # Menggunakan AI Gateway core existing
    ):
    ...
    async def process_user_query(
        ...
        if conversation_history:
            messages.extend(conversation_history[-5:])  # Context window 5 pesan terakhir
        messages.append({"role": "user", "content": payload.message})

        try:
            # Panggil AI Gateway bawaan core
            raw_ai_reply = await self.ai_gateway.generate_chat_completion(messages=messages)
            
            # Sanitasi output JSON
            cleaned_json_str = raw_ai_reply.strip()
        ...
        # 3. Handle Eskalasi jika ditandai AI
        ...
        if escalation_triggered:
            ...
            await self.escalation.trigger_escalation(
                conversation_id=0, # Dihubungkan ke ID database saat persistance aktif
