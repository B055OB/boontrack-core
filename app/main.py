...
async def get_skill_and_generate(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    email = data.email if data.email and data.email != '-' else ""
    phone = data.phone_number if data.phone_number and data.phone_number != '-' else ""
    ...
    # 3. Pencapaian & Proyek (Cetak HANYA JIKA ADA)
    if pencapaian:
