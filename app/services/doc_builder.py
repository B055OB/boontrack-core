...
def render_ats_review_docx(data: Dict[str, Any]) -> bytes:
    ...
    # 5. Weaknesses & Findings
    findings = data.get("findings") or []
    if isinstance(findings, list) and findings:
        _add_section_header(doc, "Temuan Audit & Area Peningkatan")
    ...
    if isinstance(red_flags, list) and red_flags:
        _add_section_header(doc, "Catatan Kritis & Red Flags HR", color_rgb=COLOR_WARNING)
        for rf in red_flags:
            _add_bullet_item(doc, str(rf), bold_prefix="⚠️ ")

    # 7. Rekomendasi Perbaikan Konkret (Actionable Fixes)
    fixes = data.get("actionable_fixes") or data.get("recommendations") or []
    if isinstance(fixes, list) and fixes:
        _add_section_header(doc, "Rekomendasi Perbaikan Konkret (Actionable Fixes)")
        for item in fixes:
            if isinstance(item, dict):
                section = item.get("section", "Umum")
                issue = item.get("issue") or item.get("finding") or ""
...
def render_career_pro_bundle_docx(data: Dict[str, Any]) -> bytes:
    """Merender Paket Lengkap Karir Pro: CV ATS Tailored + Rekomendasi HR Profesional + Surat Lamaran/Cover Letter."""
    doc = Document()
    _set_document_margins(doc, 0.75)
    ...
    if summary:
        _add_section_header(doc, "Professional Summary (ATS Tailored)")
    ...
    if isinstance(experiences, list) and experiences:
        _add_section_header(doc, "Professional Experience (Metric-Based ATS)")
        for exp in experiences:
            if not isinstance(exp, dict):
                continue
    ...
    if isinstance(education, list) and education:
        ...
        for edu in education:
            if isinstance(edu, dict):
                ...
                if year:
                    title_str += f" ({year})"
                _add_bullet_item(doc, title_str)
            else:
                _add_bullet_item(doc, str(edu))
    ...
    if isinstance(hr_recs, dict) and hr_recs:
        _add_section_header(doc, "Rekomendasi HR & Career Strategy", color_rgb=COLOR_SECONDARY)
    ...
    if isinstance(cl, dict) and cl:
        _add_section_header(doc, "Surat Lamaran Kerja (Cover Letter)", color_rgb=COLOR_PRIMARY)
...
def build_document_result(task_type: str, structured_data: Dict[str, Any]) -> bytes:
    """Dispatcher utama pembuat file Word (.docx) berdasarkan 4 Layanan Inti."""
    normalized = str(task_type).upper().strip()
    if normalized in ["CV_REVIEW", "ATS_DIAGNOSTIC", "ATS_REVIEW"]:
        return render_ats_review_docx(structured_data)
    elif normalized in ["CAREER_PRO_BUNDLE", "BUNDLE_CAREER", "PRO_BUNDLE"]:
        return render_career_pro_bundle_docx(structured_data)
    elif normalized in ["CV_BUILD", "CV_ATS", "CV_POLISH_REWRITE", "CV_REWRITE"]:
        return render_cv_rewrite_docx(structured_data)
    elif normalized in ["POLISH_REPHRASE", "PARAPHRASE", "DOCUMENT_POLISH"]:
