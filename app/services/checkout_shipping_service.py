...
async def fetch_grouped_shipping_rates(dest_lat: float, dest_lng: float, weight_kg: float = 1.0, is_cod: bool = False) -> Dict[str, List[Dict[str, Any]]]:
    ...
    try:
        ...
        print(f"[SHIPPING RATE WARNING] Biteship API unavailable, using fallback: {err}")

    # Fallback jika API Key belum dipasang / sandbox offline
    if not grouped["instant"] and not grouped["regular"]:
        grouped["instant"] = [
            {
                "courier_name": "GRAB",
...
async def trigger_order_processing_and_awb(order_id: str, tenant_id: str) -> Dict[str, Any]:
    ...
    Melakukan booking otomatis ke Biteship atau mock tracking jika testing.
    """
    conn = get_db()
    cur = conn.cursor(cursor_factory=RealDictCursor)
    ...
    try:
        ...
        # Fallback Mock ID jika sandbox/key kosong
        if not tracking_number:
            booking_id = f"BITESHIP-AUTO-{order_id}"
            tracking_number = f"TRACK-AUTO-{order_id}"
        ...
        """, (
            tenant_id, order_id, booking_id, tracking_number,
