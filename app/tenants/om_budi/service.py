...
OM_BUDI_SYSTEM_PROMPT = """
Anda adalah AI Operations & Member Assistant resmi untuk Om Budi (Om Budi Community & Mentorship).

Karakter & Gaya Komunikasi:
- Ramah, solutif, percaya diri, praktis, dan profesional seperti representative terpercaya Om Budi.
- Sapa member dengan hangat (misal: "Halo Kak!", "Semangat pagi!").
- Berikan informasi yang presisi berdasarkan [KNOWLEDGE BASE OM BUDI]. Jangan memberikan janji diskon atau materi di luar basis data resmi.

Segmentasi Pengguna:
- Segmen User Saat Ini: {user_segment} ({user_segment_label})

Instruksi Output:
- Jika user meminta bicara dengan CS/Admin atau menggunakan kata eskalasi refund/kendala berat, set "is_escalated": true.
...
class OmBudiService:
    ...
    def _load_knowledge(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("[OM_BUDI] Failed to load knowledge base: %s", e)
        return {"programs": [], "faq": []}

    def detect_segment(self, phone_number: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        Logika segmentasi user (bisa diintegrasikan dengan query Supabase 'members' table).
        """
    ...
    async def handle_incoming_message(
        ...
        # 1. Immediate Rule-based Escalation Check
        ...
        # 2. AI Prompt Synthesis with Knowledge Base
        system_instruction = OM_BUDI_SYSTEM_PROMPT.format(
        ...
        try:
            ...
            logger.error("[OM_BUDI AI ERROR] %s", e, exc_info=True)
            return {
                "reply": f"Halo Kak {user_name}! Terima kasih sudah menghubungi Om Budi Support. Ada yang bisa kami bantu terkait program mentorship atau jadwal sesi?",
                "intent": "GENERAL",
                "is_escalated": False,
