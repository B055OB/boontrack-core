...
# Unified router under /api/v1/whatsapp/config
router = APIRouter(prefix="/api/v1/whatsapp/config", tags=["WhatsApp Infrastructure Control"])
...
def register_whatsapp_control_routes(app: web.Application):
    # Register aiohttp routes under the unified path
    app.router.add_get("/api/v1/whatsapp/config/status", aiohttp_get_whatsapp_status)
    app.router.add_post("/api/v1/whatsapp/config/reconnect", aiohttp_trigger_reconnect)
    ...
    logger.info("[ROUTER] WhatsApp Control routes registered under /api/v1/whatsapp/config (with legacy aliases).")