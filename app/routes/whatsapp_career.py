...
from app.core.database import count_referrals
...
def get_whatsapp_full_menu(sender_wa_id: str) -> str:
    """Menampilkan 5 menu lengkap BoonTrack langsung tanpa syarat."""
    ...
    return (
        ...
        "2️⃣ *Review & Cek Skor ATS CV* (Upload PDF/DOCX)\n"
...
async def handle_incoming_whatsapp(request: web.Request) -> web.Response:
    ...
    # --- POST_CV STATE (Setelah Review/Buat CV) ---
    if current_mode == "post_cv":
        ...
        # Tanya karir bebas di mode post_cv
    ...
    # Fallback jika user langsung mengetik pertanyaan bebas
    ai_reply = await ai_gateway.generate(user_message=user_text, context={"user_id": sender_wa_id, "feature": "career_consultation"})
    if ai_reply:
        await send_whatsapp_text(sender_wa_id, ai_reply)
    else:
